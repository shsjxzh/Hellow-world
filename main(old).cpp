...
int fp = 1; int sp = SIZE;      //fp代表堆空间的帧指针，地址由低到高； sp代表栈空间的的栈指针，地址由高到低。
                                //注意fp指针指向的是空的内存
//此处是指令类型
...
private:
	struct product {
		...
		int answer_of_stage_3[2] = { 0, 0 };                 //第三步数据暂存, 可能是一个布尔变量，可能是一个内存地址，可能是算术数(低32位和高32位)
	...
	void add(product& obj) {
		if (obj.instruct.no_of_reg[2] != -1) obj.answer_of_stage_3[0] = obj.data_from_reg[0] + obj.data_from_reg[1];
		else obj.answer_of_stage_3[0] = obj.data_from_reg[0] + obj.instruct.x;
	}
	void addu(product& obj) {
		if (obj.instruct.no_of_reg[2] != -1) obj.answer_of_stage_3[0] = obj.data_from_reg[0] + obj.data_from_reg[1];
		else obj.answer_of_stage_3[0] = obj.data_from_reg[0] + obj.instruct.x;
	}
	void addiu(product& obj) { obj.answer_of_stage_3[0] = obj.data_from_reg[0] + obj.instruct.x; }
	...
	void mul(product& obj) {
		if (obj.instruct.no_of_reg[0] != -1) {
			...
			obj.answer_of_stage_3[0] = int(tmp);
			obj.answer_of_stage_3[1] = int(tmp >> 32);
		}
	}
	void mulu(product& obj) {
		if (obj.instruct.no_of_reg[0] != -1) {
			if (obj.instruct.no_of_reg[2] != -1) obj.answer_of_stage_3[0] = unsigned(obj.data_from_reg[0]) * unsigned(obj.data_from_reg[1]);
			else obj.answer_of_stage_3[0] = unsigned(obj.data_from_reg[0]) * unsigned(obj.instruct.x);
		}
		else {
			long long tmp;
	...
	void div(product& obj) {
		if (obj.instruct.no_of_reg[0] != -1) {
			...
			if (obj.instruct.no_of_reg[2] != -1) {
				obj.answer_of_stage_3[0] = obj.data_from_reg[0] / obj.data_from_reg[1];
				obj.answer_of_stage_3[1] = obj.data_from_reg[0] % obj.data_from_reg[1];
			}
			else {
				obj.answer_of_stage_3[0] = obj.data_from_reg[0] / obj.instruct.x;
				obj.answer_of_stage_3[1] = obj.data_from_reg[0] % obj.instruct.x;
			}
		}
	}
	void divu(product& obj) {
		if (obj.instruct.no_of_reg[0] != -1) {
			...
			if (obj.instruct.no_of_reg[2] != -1) {
				obj.answer_of_stage_3[0] = unsigned(obj.data_from_reg[0]) / unsigned(obj.data_from_reg[1]);
				obj.answer_of_stage_3[1] = unsigned(obj.data_from_reg[0]) % unsigned(obj.data_from_reg[1]);
			}
			else {
				obj.answer_of_stage_3[0] = unsigned(obj.data_from_reg[0]) / unsigned(obj.instruct.x);
				obj.answer_of_stage_3[1] = unsigned(obj.data_from_reg[0]) % unsigned(obj.instruct.x);
			}
		}
	}
	...
	void neg(product& obj) { obj.answer_of_stage_3[0] = -obj.data_from_reg[0]; }     //neg是取负号吗？
	void negu(product& obj) { obj.answer_of_stage_3[0] = unsigned(-obj.data_from_reg[0]); }
	...
	void li(product& obj) { obj.answer_of_stage_3[0] = obj.instruct.x; }
	...
	void b(product& obj) { obj.answer_of_stage_3[0] = true; }
	...
	void j(product& obj) { obj.answer_of_stage_3[0] = true; }
	void jr(product& obj) { obj.answer_of_stage_3[0] = true; }
	void jal(product& obj) { obj.answer_of_stage_3[0] = true;}
	void jalr(product& obj) { obj.answer_of_stage_3[0] = true; }
	void la(product& obj) {
		if (obj.instruct.label >= 0) obj.answer_of_stage_3[0] = obj.instruct.label;
		else obj.answer_of_stage_3[0] = obj.data_from_reg[0] + obj.instruct.x;
	}
	...
	void sb(product& obj) {
		if (obj.instruct.label >= 0) obj.answer_of_stage_3[0] = obj.instruct.label;
		else obj.answer_of_stage_3[0] = obj.data_from_reg[1] + obj.instruct.x;
	}
	void sh(product& obj) { sb(obj); }
	void sw(product& obj) { sb(obj); }
	void move(product& obj) { obj.answer_of_stage_3[0] = obj.data_from_reg[0]; }
	void mfhi(product& obj) { obj.answer_of_stage_3[0] = obj.data_from_reg[0]; }
	void mflo(product& obj) { obj.answer_of_stage_3[0] = obj.data_from_reg[0]; }
	...
	void Write_Bk() {
		if (!finish && can_read[4]) {
			...
			if (23 <= num && num <= 39) {   //跳转指令
				--control_hazard;
				if (order[4].answer_of_stage_3[0]) {
				...
				for (int i = 0; i <= 1; ++i) {
					if (order[4].reg_to_be_input[i] != -1) {
						rgstr[order[4].reg_to_be_input[i]] = order[4].answer_of_stage_3[i];
	...
	void Memory_Access() {
		if (!finish && can_read[3]) {
			...
			if (40 <= num && num <= 43) {    //load命令
				if (num == 40) order[3].data_from_Memory = order[3].answer_of_stage_3[0];
				else if (num == 41) order[3].data_from_Memory = Memory[order[3].answer_of_stage_3[0]];
				else if (num == 42) order[3].data_from_Memory = *((short*)(Memory + order[3].answer_of_stage_3[0]));
				else if (num == 43) order[3].data_from_Memory = *((int*)(Memory + order[3].answer_of_stage_3[0]));

			}
			else if (44 <= num && num <= 46) {  //store
				if (num == 44) Memory[order[3].answer_of_stage_3[0]] = order[3].data_from_reg[0];
				else if (num == 45) new (static_cast<void*> (&Memory[order[3].answer_of_stage_3[0]])) short(order[3].data_from_reg[0]);  //访问内存
				else if (num == 46) new (static_cast<void*> (&Memory[order[3].answer_of_stage_3[0]])) int(order[3].data_from_reg[0]);    //访问内存
			}
			else if (num == 51) {
				if (order[3].answer_of_stage_3[1] == -10) {
					char tmp[2000];
					cin.getline(tmp, order[3].data_from_reg[1]);
					int len = strlen(tmp);
...
void performer::syscall(performer::product& obj) {  //记得改回引用
	...
	case 4:
		...
		break;
	case 5:cin >> obj.answer_of_stage_3[0]; break;
	case 8:
		obj.answer_of_stage_3[1] = -10; break;//取一个特殊值来使其在第4阶段执行内存访问
	case 9:
		if (fp % 4 != 0) {
			int k = fp / 4 + 1;
			fp = k * 4;
		}
		obj.answer_of_stage_3[0] = fp;
...
void initialize() {
	...
	opr["remu"] = 21315;
	opr["li"] = 92216;
	opr["seq"] = 13317; opr["sge"] = 13318;
	opr["sgt"] = 13319; opr["sle"] = 13320;
	opr["slt"] = 13321; opr["sne"] = 13322;
...
void Input(char * a)
{
	...
	*/
	...
	//ifstream fin("function_test-huyuncong.s");
	initialize();
